...
> **One-line summary:** In Chassis, insert components and apply text styles. For standalone text, always use **Basic Text Asset** — never `figma.createText()`. Never hardcode font family names in `loadFontAsync` — resolve them at runtime from `textNode.fontName` ("from") and `style.fontName` ("to"). Never call `setExplicitVariableModeForCollection` on any node. Never write raw typography properties (`fontName`, `fontSize`, etc.).
