...
| ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `instance.setExplicitVariableModeForCollection(collection, modeId)` on a component instance | Chassis components resolve variable modes (themes, fonts, colors) automatically from the library. Setting modes manually on instances overrides the library context and causes wrong font and color resolution — the component renders its literal default values instead of the brand-resolved ones. | Set variable modes only on **wrapper frames** for whole-section/screen theme switching. Never on individual component instances.                      |
...
| `figma.createText()` for standalone text in Chassis                                         | Raw text nodes have no Chassis token bindings, no theme response, and no Asset Override system. They look identical at placement time but break on theme switch and diverge from the design system.                                                                                    | Always use **Basic Text Asset** (`cx.asset.text` — component key `6209e2b0b166983bcb5697be17578479f8bcbfcb`). Import it with `importComponentByKeyAsync`, create an instance, set `text#142:1` via `setProperties()`, then apply a `font/*` style via `setTextStyleIdAsync` on the inner TEXT node. |
...
> **One-line summary:** In Chassis, insert components and apply text styles. For standalone text, always use **Basic Text Asset** — never `figma.createText()`. Never hardcode font family names in `loadFontAsync` — resolve them at runtime from `textNode.fontName` ("from") and `style.fontName` ("to"). Never set variable modes on instances. Never write raw typography properties (`fontName`, `fontSize`, etc.).
